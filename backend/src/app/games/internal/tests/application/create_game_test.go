...
func Test_it_should_be_able_to_create_a_game_without_a_cover(t *testing.T) {
	repository := &game_doubles_test.MemoryGameRepository{}
	fileSystem := game_doubles_test.NewMockFileSystem()
	factory := game_factories.NewDefaultGameFactory(fileSystem)
	imageUploader := shared_services.NewLocalImageUploader(fileSystem)
	...
	if len(fileSystem.ListFilesForTest()) < 0 {
		t.Error("Expected game file to exist in the file system, but it does not.")
		return
	}
}
...
func Test_it_not_should_rollback_copied_image_if_game_creation_fails_given_no_cover(t *testing.T) {
	repository := &game_doubles_test.MemoryGameRepository{}
	fileSystem := game_doubles_test.NewMockFileSystem()
	factory := game_factories.NewDefaultGameFactory(fileSystem)
	imageUploader := shared_services.NewLocalImageUploader(fileSystem)
