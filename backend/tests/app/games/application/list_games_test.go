...
import (
	list_games "retrolauncher/backend/src/app/games/application"
...
func Test_it_should_be_able_to_list_games(t *testing.T) {
	...
	sut := list_games.NewListGames(repository, imageUploader)

	result := sut.Execute(list_games.ListGamesInput{
		Name: "",
	})
