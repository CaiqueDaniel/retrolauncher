// Cynhyrchwyd y ffeil hon yn awtomatig. PEIDIWCH Â MODIWL
// This file is automatically generated. DO NOT EDIT
import {game_controller} from '../models';
import {application} from '../models';

export function AutoIndexGames():Promise<void>;

export function Create(arg1:game_controller.CreateInputDto):Promise<Array<string>>;

export function Get(arg1:game_controller.GetInputDto):Promise<application.GetGameOutput>;

export function GetPlatformTypes():Promise<Array<string>>;

export function List(arg1:game_controller.ListInputDto):Promise<Array<application.ListGamesOutput>>;

export function StartGame(arg1:game_controller.GetInputDto):Promise<void>;

export function Update(arg1:game_controller.UpdateInputDto):Promise<Array<string>>;
